...
from .serializers import TargetListSerializer, TargetSerializer
...
class TargetFilter(filters.FilterSet):
    ...
    project_start_date = filters.DateFilter(field_name = 'project_start_date__date', lookup_expr = 'gte')
    project_due_date = filters.DateFilter(field_name = 'project_due_date__date', lookup_expr = 'lte')
...
class TargetList( CustomLoginRequiredMixin,generics.ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        self.queryset = Target.objects.exclude( status='deleted').all().filter(user_id__team = request.login_user.team).order_by('-id')
        ...
        self.search_fields = ['status','type', 'cohort', 'project_start_date', 'project_due_date', 'project_student_name']
